...
void ProcessFrame(IplImage *frame, IplImage *outFrame, IplImage* filter, IplImage* freq, FilterType t, int *_r)
{
  ...
  for (int i=0; i<size; i++)
  {
    inpG[2*i] = pow(-1.0,i/w+i%w) * ((unsigned char*)frame->imageData)[i*3 + 1];
    inpG[2*i+1] = 0.0;
  }

  for (int i=0; i<size; i++)
  {
    inpR[2*i] = pow(-1.0,i/w+i%w) * ((unsigned char*)frame->imageData)[i*3 + 2];
    inpR[2*i+1] = 0.0;
  }
  ...
  for (int i=0; i<size; i++)
    ...
    {
      maxR = minR = log( sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1]));
      break;
    }
  ...
  {
    if (sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1]) != 0)
      if(maxR < log( sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1]) ) )
        maxR = log( sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1]) );
  ...
  for (int i=0; i<size; i++)
    if (sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) != 0)
    {
      maxG = minG = log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) );
      break;
    }
  ...
  {
    if (sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) != 0)
      if(maxG < log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) ) ) 
        maxG = log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) ) ;

    if (sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) != 0)
      if(minG > log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) ) ) 
        minG = log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) ) ;
  }

  for (int i=0; i<size; i++)
    if (sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) != 0)
    {
      maxB = minB = log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]));
      break;
    }
  ...
  {
    if (sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) != 0)
      if(maxB < log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) ) ) 
        maxB = log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) );

    if (sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) != 0)
      if(minB > log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) ) ) 
        minB = log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) );
  }

  //Show frequency area with filter
  for (int i=0; i<size; i++)
    if (sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1]) != 0)
      freq->imageData[3*i+0] = (char) (255 * ( (log( sqrt(outR[2*i]*outR[2*i]+outR[2*i+1]*outR[2*i+1])) - minR ) / (maxR - minR) ) );
    else
      freq->imageData[3*i+0] = 0;

  for (int i=0; i<size; i++)
    if (sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1]) != 0)
      freq->imageData[3*i+1] = (char) (255 * ( (log( sqrt(outG[2*i]*outG[2*i]+outG[2*i+1]*outG[2*i+1])) - minG ) / (maxG - minG) ) );
    else
      freq->imageData[3*i+1] = 0;

  for (int i=0; i<size; i++)
    if (sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1]) != 0)
      freq->imageData[3*i+2] = (char) (255 * ( (log( sqrt(outB[2*i]*outB[2*i]+outB[2*i+1]*outB[2*i+1])) - minB ) / (maxB - minB) ) );
    else
  ...
  for (int i=0; i<size; i++)
    outFrame->imageData[3*i+0] = (char) (pow(-1.0,i/w+i%w) * inpB[2*i]);

  for (int i=0; i<size; i++)
    outFrame->imageData[3*i+1] = (char) (pow(-1.0,i/w+i%w) * inpG[2*i]);

  for (int i=0; i<size; i++)
    outFrame->imageData[3*i+2] = (char) (pow(-1.0,i/w+i%w) * inpR[2*i]);
